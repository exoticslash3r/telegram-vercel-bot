...
const ADMIN_ID = Number(process.env.ADMIN_ID); // set admin ID in Vercel env
if (!BOT_TOKEN) throw new Error("BOT_TOKEN not set in environment variables");

const bot = new Telegraf(BOT_TOKEN);

// In-memory missions storage (you can replace with DB)
let MISSIONS = {};

// Helper function for user label
const userLabel = (ctx) => ${ctx.from.first_name} ${ctx.from.last_name || ""};
