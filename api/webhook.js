...
if (!BOT_TOKEN  !ADMIN_ID  !PUBLISH_CODE) throw new Error("Missing env variables");

const bot = new Telegraf(BOT_TOKEN);

// Use session for user flows
bot.use(session());
...
bot.on("text", async (ctx) => {
  ...
  if (ctx.session.flow === "mission_proof") {
  ...
  if (ctx.session.flow === "publish_code") {
